...
class Product {
	...
	function insertProduct($params) {
		...
		if(!empty($_FILES))
		{
			if (!file_exists('../../../products/'.$params["vendorid"])) 	
				mkdir('../../../products/'.$params["vendorid"], 0755, true);	

			if(is_uploaded_file($_FILES['file']['tmp_name']))
			{
				$filename = isset($_FILES['file']["name"]) != '' ? $_FILES['file']["name"] : '';
				$result = move_uploaded_file($_FILES['file']['tmp_name'], '../../../products/'.$params["vendorid"].'/'. $filename);
			}
		}
...
/*		if( $filename != '' ){
	...
	function getRecords($params) {
		...
		$where .=" WHERE vendorid = '".$params["vendorid"]."'";
