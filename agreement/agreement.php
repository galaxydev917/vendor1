...
/*		$date = new DateTime($signed_date);
		...
		$sql = "Update `vendor` set signed_date='" . $signed_date."', signed=1 WHERE vendorid = '".$_SESSION["vendorid"]."' ";
		//}		
	...
	function getLoadData() {
		$sql = "select vendorid, signed, signed_date from vendor where vendorid = '".$_SESSION["vendorid"]."' LIMIT 1";
	...
	function updateRegNum($params) {
		$data = array();
		$sql = "Update `vendor` set reg_receipt='" . $params["receipt_number"]."' WHERE vendorid = '".$_SESSION["vendorid"]."'";
		$result = mysqli_query($this->conn, $sql) or die("error to update owner data");
